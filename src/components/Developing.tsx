...
const DEVELOPING_REFRESH_COST = 12
const DEVELOPING_CACHE_KEY = 'open-news-developing-cache-v1'
const AUTO_REFRESH_MS = 60 * 60 * 1000

async function fetchDevelopingNews(): Promise<{ items: NewsItem[]; fromCache: boolean }> {
    return fetchTavilyNewsCached(
        'developing news stories ongoing events today',
        DEVELOPING_REFRESH_COST,
        DEVELOPING_CACHE_KEY,
        AUTO_REFRESH_MS,
    )
}
...
export default function Developing({ consume }: DevelopingProps) {
    ...
    const load = useCallback(async () => {
        ...
        setError(null)
        try {
            const { items: news, fromCache } = await fetchDevelopingNews()
